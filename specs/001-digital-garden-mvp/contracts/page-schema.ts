/**
 * contracts/page-schema.ts
 *
 * Astro Content Collection schema for all garden pages.
 * This file is the source-of-truth contract between content authors and the build.
 * Copied verbatim into src/content/config.ts during implementation.
 *
 * Validated at build time by Astro. Any frontmatter that violates this schema
...
/**
 * CVE-specific fields. Activated when `cveId` is present.
 * All CVE fields are required when `cveId` is set (validated via .superRefine).
 */
const cveExtension = z.object({
  /** CVE identifier. Must match CVE-YYYY-NNNNN format. */
  cveId: z
    .string()
    .regex(/^CVE-\d{4}-\d{4,}$/, { message: "Must match CVE-YYYY-NNNNN format" })
    .optional(),
...
export const collections = {
  ...
  garden: defineCollection({ schema: gardenPageSchema }),
};
