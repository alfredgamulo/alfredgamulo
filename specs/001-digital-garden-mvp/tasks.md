...
- [ ] T005 [P] Update `Justfile` to replace generic shell detection with Astro-specific targets: `build` → `npm run build`, `dev` → `npm run dev`, `test` → `npm audit --audit-level=high && eslint src/ && npx playwright test && node tests/seo/smoke-check.mjs`, `deploy` → `bash scripts/deploy.sh` in `Justfile`
- [ ] T005b [P] Create `eslint.config.mjs` using flat config with `@eslint/js`, `typescript-eslint`, and `eslint-plugin-astro` plugins; configure TypeScript-aware parsing for `src/**/*.{ts,astro}`; install these as dev dependencies via `npm install -D eslint-plugin-astro typescript-eslint @eslint/js` in `eslint.config.mjs`
...
- [ ] T008 [P] Create `src/styles/global.css`: CSS custom properties for color palette (12 tag colors), typography scale, dark mode via `@media (prefers-color-scheme: dark)`, and base reset; include `pre, code { overflow-x: auto; max-width: 100%; }` for FR-008 horizontal scroll on code blocks in `src/styles/global.css`
...
- [ ] T037 Run `just build && just test` and confirm all e2e tests pass (the `just test` target now includes lint, npm audit, Playwright, and SEO smoke-check per T005)
